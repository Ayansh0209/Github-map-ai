...
export interface FunctionNode {
    ...
    // ── Phase 4: Retrieval signals (optional — backward compatible) ───────────
    // Detected during AST traversal in functionLevel.ts at no extra cost.
    // Used by the retrieval index to help AI decide which functions to fetch.
    hasAuthCheck?: boolean;       // true if function contains auth/permission/role logic
    hasDatabaseCall?: boolean;    // true if function contains ORM/query/database calls
}
...
export interface FileNode {
    ...
    entryReasons?: string[];       // human-readable audit trail e.g. ["package.json:main +20"]
    ...
    // ── Phase 4: Retrieval signals (optional — backward compatible) ───────────
    // Detected during AST traversal at no extra cost.
    // Used by the retrieval index and AI issue mapping.
    isBarrel?: boolean;            // true if file only re-exports, no implementations
    barrelTargets?: string[];      // if barrel, the real implementation fileIds it points to
    semanticRole?: string;         // architectural role: resolver, auth, mutation, util, etc.
}
